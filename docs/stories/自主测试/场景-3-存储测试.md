...
> **导航**: [← 场景-2-接口测试](./场景-2-接口测试.md) · [场景-4-错误边界 →](./场景-4-错误边界.md)
...
flowchart TB
    ...
    subgraph SESSION_LIFECYCLE["会话生命周期"]
        direction TB
        INIT["initialize()<br/>→ loadBackendSessions (可选)"]:::exec
        GEN["*_generateUUID()<br/>→ UUID v4"]:::exec
        CREATE["createSession(id, pageInfo)<br/>→ session object with key+tags+title"]:::exec
        SAVE["saveSession(id, force?)<br/>→ 更新 updatedAt + syncSessionToBackend"]:::exec
        GET["getSession(id) → session | null"]:::exec
        GET_ALL["getAllSessions()<br/>→ 按 id 去重 + 按 favorite/name 排序"]:::exec
        ACTIVATE["activateSession(id)<br/>→ 保存当前 + 切换新 + 更新页面信息"]:::exec
        DELETE["deleteSession(id)<br/>→ 删除本地 + 后端 + aicr 回调"]:::exec
        DUPLICATE["duplicateSession(id, data?)<br/>→ 深拷贝 messages/tags + 新 UUID"]:::exec
        SEARCH["searchSessions(query, limit?)<br/>→ 搜索 title/url + 后端合并"]:::exec
    end
...
flowchart LR
    ...
    subgraph infra["基础设施"]
        direction TB
        SETUP["setup.mjs<br/>chrome.storage mock (Map)<br/>chrome.runtime mock<br/>setChromeError/invalidate"]:::exec
    end
    subgraph storage["存储层测试"]
        direction TB
        STO_T["storage.test.mjs<br/>70 用例<br/>mock 基础 + StorageUtils<br/>+ StorageHelper<br/>+ SessionManager"]:::exec
        STO2["storage.test.mjs (core)<br/>5 用例<br/>IIFE 加载验证"]:::exec
        MCK["chrome.test.mjs<br/>4 用例<br/>mock 正确性"]:::exec
    end
    SETUP --> storage
```
...
| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| v1.0.0 | 2026-06-02 | coder | 初始版本：数据持久化测试场景文档 |
