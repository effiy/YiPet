...
;(function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      DomHelper.runPageScriptWithData({
        ...
        .catch(function (eventOrError) {
          loading = false
          var errorMsg =
            eventOrError && eventOrError.detail && eventOrError.detail.error
              ? eventOrError.detail.error
              : eventOrError && eventOrError.message
  ...
  function renderBlocks(container) {
    ...
    var mermaidCodeBlocks = container.querySelectorAll(
      'code.language-mermaid, code.language-mmd, pre code.language-mermaid, pre code.language-mmd, code[class*="mermaid"]',
    )
        ...
        unprocessed.forEach(function (element, index) {
          ...
          if (element.tagName === 'DIV' && element.classList.contains('mermaid')) {
            ...
            if (!element.hasAttribute('data-mermaid-source'))
              element.setAttribute('data-mermaid-source', mermaidContent)
  ...
  function renderSingleBlock(mermaidId, mermaidDiv, mermaidContent) {
    DomHelper.runPageScriptWithData({
      ...
      isSuccess: function (e) {
        return e && e.detail && e.detail.id === mermaidId
      },
    })
      .then(function (event) {
        if (!event.detail.success) {
          ...
          setTimeout(function () {
            addActions(mermaidDiv, event.detail.svgContent || '', mermaidContent)
          }, 100)
        }
      })
      .catch(function () {
  ...
  function showMermaidError(mermaidDiv, mermaidContent) {
    ...
    errorDiv.innerHTML =
      '<div>❌ Mermaid 图表渲染失败</div><pre style="font-size:10px;margin-top:5px;overflow-x:auto;">' +
      escaped +
      '</pre>'
  ...
  function addActions(mermaidDiv, svgContent, mermaidSourceCode) {
    ...
    downloadButton.addEventListener('click', function (e) {
      e.stopPropagation()
      e.preventDefault()
      getSvgContent(mermaidDiv, svgContent)
        .then(function (svg) {
  ...
  function getSvgContent(mermaidDiv, fallbackSvg) {
    return new Promise(function (resolve) {
      ...
      script.textContent =
        '(function(){var d=document.getElementById("' +
        mermaidDiv.id +
        '");if(d){var s=d.querySelector("svg");if(s){var c=s.cloneNode(true);c.getAttribute("xmlns")||c.setAttribute("xmlns","http://www.w3.org/2000/svg");window.postMessage({type:"mermaid-svg-content",id:"' +
        mermaidDiv.id +
        '",svgContent:new XMLSerializer().serializeToString(c)},"*")}})()'
        ...
        try {
          ...
          img.onload = function () {
            ...
            canvas.toBlob(function (blob) {
              ...
              else tryStringMethod(svgString, w, h, resolve, reject)
            }, 'image/png')
          }
          img.onerror = function () {
            tryStringMethod(svgString, w, h, resolve, reject)
          }
          img.src = svgUri
          return
        } catch (e) {}
  ...
  function tryStringMethod(svgString, defaultW, defaultH, resolve, reject) {
    ...
    try {
      ...
      var svgUri =
        'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(new XMLSerializer().serializeToString(svgEl))
      var img = new Image()
      img.onload = function () {
        var canvas = document.createElement('canvas')
        var ctx = canvas.getContext('2d')
  ...
  function openFullscreen(mermaidDiv, mermaidSourceCode) {
    ...
    if (!clonedMermaid.querySelector('svg')) {
      var content =
        mermaidSourceCode || clonedMermaid.getAttribute('data-mermaid-source') || clonedMermaid.textContent || ''
      if (content.trim()) {
        ...
        setTimeout(function () {
          DomHelper.runPageScriptWithData({
            ...
            cleanupDelayMs: 1000,
            isSuccess: function (e) {
              return e && e.detail && e.detail.id === clonedId
            },
          })
        }, 100)
      }
