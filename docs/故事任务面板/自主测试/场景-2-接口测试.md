...
> **导航**: [← 前驱](./场景-1-核心逻辑.md) | [后继 →](./场景-3-存储测试.md)
...
flowchart LR
    subgraph API["RequestClient 方法"]
        GET["get(url, params, opts)"]:::action
        POST["post(url, data, opts)"]:::action
        PUT["put(url, data, opts)"]:::action
        DELETE["delete(url, opts)"]:::action
        RAW["request(opts)"]:::action
    end
    subgraph CORE["request() 核心"]
        MERGE["合并 defaultOptions"]:::op
        URL["_buildUrlWithParams"]:::op
        BODY["_buildFetchOptions<br/>JSON.stringify(data)"]:::op
        FETCH["_fetchWithRetry<br/>fetch() + Promise.race(timeout)"]:::op
    end
    GET --> RAW
    POST --> RAW
    PUT --> RAW
...
flowchart LR
    subgraph infra["基础设施"]
        SETUP["setup.mjs<br/>mockFetch = vi.fn()<br/>AbortController (jsdom)<br/>loadModule IIFE 加载"]:::setup
    end
    subgraph mocks["fetch mock 策略"]
        JSON["mockJsonResponse<br/>{ok, json: => data}"]:::mock
        TEXT["mockTextResponse<br/>{ok, text: => str}"]:::mock
        ERR["mockFetchError<br/>throw TypeError"]:::mock
        TIMEOUT["永不 resolve<br/>+ vi.useFakeTimers"]:::mock
    end
    subgraph tests["测试文件"]
        API_T["api.test.mjs<br/>🆕 27 用例"]:::test
        REQ_T["request.test.mjs<br/>🆕 27 用例"]:::test
        REQ2["request.test.mjs (core)<br/>🆕 11 用例"]:::test
    end
    SETUP --> mocks --> tests
```
