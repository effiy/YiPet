...
flowchart TB
    subgraph external["外部依赖（运行时 CDN + 库文件）"]
        VUE["Vue 3 CDN<br/>libs/vue.global.js<br/>─ 失效影响: 🔴 全部 Vue 组件不可渲染<br/>─ 影响 9 个组件 + Popup"]:::externalP0
        MARKED["marked.js<br/>libs/marked.min.js<br/>─ 失效影响: 🟡 AI 回复显示原始 Markdown<br/>─ 影响聊天窗口 · 编辑器预览"]:::externalP1
        TURNDOWN["turndown.js<br/>libs/turndown.js<br/>─ 失效影响: 🟡 HTML→Markdown 转换失败<br/>─ 影响编辑器粘贴 · 页面抓取"]:::externalP1
        MERMAID["Mermaid CDN<br/>libs/mermaid.min.js + cdn/<br/>─ 失效影响: 🟢 图表区域显示空白<br/>─ 影响 Mermaid 渲染（懒加载 · 非核心）"]:::externalP2
        MD5["md5.js<br/>libs/md5.js<br/>─ 失效影响: 🟢 哈希计算不可用<br/>─ 影响少量辅助功能"]:::externalP2
        TAILWIND["Tailwind CSS<br/>assets/styles/tailwind.css<br/>─ 失效影响: 🟡 样式退化到无样式状态<br/>─ 影响全部 UI 组件"]:::externalP1
        MARKDOWN_CDN["cdn/markdown/markdown.js<br/>─ 失效影响: 🟡 marked 封装不可用<br/>─ 影响 markdown 解析封装"]:::externalP1
    end

    subgraph manifest["manifest.json — 加载顺序 = 依赖顺序"]
        M0["core/config.js<br/>PET_CONFIG 全局配置"]:::manifestRoot
        M1["libs/md5.js"]:::manifestLib
        M2["core/utils/api/token.js"]:::manifestCore
        M3["core/utils/api/request.js"]:::manifestCore
        M4["core/api/core/ApiManager.js<br/>依赖 token + request"]:::manifestCore
        M5["libs/vue.global.js<br/>🔴 所有 Vue 组件的前置依赖"]:::manifestVue
        M6["core/bootstrap/bootstrap.js<br/>StorageHelper · 位置计算"]:::manifestCore
        M7["modules/pet/content/core/petManager.core.js<br/>依赖 PET_CONFIG + StorageHelper"]:::manifestEntry
    end

    subgraph internal["内部模块依赖关系"]
        PM["PetManager 类<br/>≈ 32 个子模块 via prototype 挂载"]:::internalCore
        VUECMP["Vue 组件 9 个<br/>ChatWindow · ChatHeader · ChatInput · ChatMessages<br/>TokenSettingsModal · AiSettingsModal<br/>SessionTagManager · FaqManager · FaqTagManager"]:::internalVue
        SW["Service Worker<br/>register.js → MessageRouter<br/>→ PetHandler · MessageForwardHandler<br/>→ WeWorkHandler · TabHandler<br/>→ ExtensionHandler"]:::internalSW
        POPUP["PopupController<br/>→ chrome.runtime.sendMessage<br/>→ chrome.storage.local"]:::internalPopup
    end
    ...
    classDef externalP0 fill:#ffcdd2,stroke:#c62828
    classDef externalP1 fill:#fff9c4,stroke:#f9a825
    classDef externalP2 fill:#e0f7fa,stroke:#00838f
    classDef manifestRoot fill:#c8e6c9,stroke:#1b5e20
    classDef manifestLib fill:#e8eaf6,stroke:#283593
    classDef manifestCore fill:#e3f2fd,stroke:#1565c0
    classDef manifestVue fill:#ffcdd2,stroke:#c62828
    classDef manifestEntry fill:#e8f5e9,stroke:#2e7d32
    classDef internalCore fill:#c8e6c9,stroke:#2e7d32
    classDef internalVue fill:#fce4ec,stroke:#c62828
    classDef internalSW fill:#eceff1,stroke:#37474f
    classDef internalPopup fill:#e0f7fa,stroke:#00838f
```
...
flowchart TB
    subgraph triggers["变更触发源"]
        T1["manifest.json<br/>content_scripts 顺序变更"]:::triggerP0
        T2["PET_CONFIG (config.js)<br/>配置结构变更"]:::triggerP0
        T3["PetManager 类<br/>构造函数/prototype 接口变更"]:::triggerP0
        T4["ApiManager<br/>请求拦截器接口变更"]:::triggerP0
        T5["TokenManager<br/>获取方式/存储 key 变更"]:::triggerP1
        T6["RequestClient<br/>请求配置接口变更"]:::triggerP1
        T7["Vue 组件<br/>props/events 接口变更"]:::triggerP1
        T8["chrome.storage.local<br/>Key 命名变更"]:::triggerP1
        T9["样式文件<br/>CSS class 变更"]:::triggerP2
    end

    subgraph impacts["爆炸半径"]
        I1["manifest 顺序变更<br/>→ 全部 88 个文件的加载稳定性<br/>→ InjectionService.CONTENT_SCRIPT_FILES 需同步"]:::impact
        I2["config 变更<br/>→ 37 个引用 PET_CONFIG 的模块<br/>→ config.js 全局赋值逻辑"]:::impact
        I3["PetManager 接口变更<br/>→ 32 个子模块 prototype 挂载<br/>→ 9 个 Vue 组件调用<br/>→ PopupController 引用<br/>→ Service Worker 引用"]:::impact
        I4["ApiManager 变更<br/>→ SessionService · FaqService<br/>→ 所有 API 调用链路"]:::impact
        I5["TokenManager 变更<br/>→ ApiManager 默认拦截器<br/>→ TokenSettingsModal<br/>→ Popup 认证逻辑"]:::impact
        I6["RequestClient 变更<br/>→ ApiManager<br/>→ 所有 HTTP 请求"]:::impact
        I7["Vue 组件接口变更<br/>→ ChatWindow 复合组件树<br/>→ Popup Vue 组件"]:::impact
        I8["storage Key 变更<br/>→ PetManager.state · PopupController<br/>→ SW register.js · TokenManager"]:::impact
        I9["CSS 变更<br/>→ 7 个样式文件交叉引用<br/>→ 组件内联样式"]:::impact
    end
    ...
    classDef triggerP0 fill:#ffcdd2,stroke:#c62828
    classDef triggerP1 fill:#fff9c4,stroke:#f9a825
    classDef triggerP2 fill:#e0f7fa,stroke:#00838f
...
```mermaid
flowchart TB
    START["Content Script 开始加载"]:::src --> C1{"PET_CONFIG<br/>已定义?"}:::check
    C1 -->|"❌"| W1["⚠️ PET_CONFIG 未定义<br/>使用默认值"]:::warn
    C1 -->|"✅"| C2{"Vue.createApp<br/>已定义?"}:::check

    C2 -->|"❌"| W2["🔴 Vue 不可用<br/>ChatWindow 等 9 个组件全部跳过<br/>仅保留原生 DOM 宠物渲染"]:::critical
    C2 -->|"✅"| C3{"typeof marked<br/>=== 'function'?"}:::check

    C3 -->|"❌"| W3["🟡 marked 不可用<br/>AI 回复显示原始 Markdown"]:::warn
    C3 -->|"✅"| C4{"TurndownService<br/>已定义?"}:::check

    C4 -->|"❌"| W4["🟡 turndown 不可用<br/>编辑器粘贴降级"]:::warn
    C4 -->|"✅"| C5{"chrome.storage.local<br/>可用?"}:::check

    C5 -->|"❌"| W5["🟡 storage 不可用<br/>状态不持久化 · Token 从环境变量获取"]:::warn
    C5 -->|"✅"| C6{"chrome.runtime.id<br/>有效?"}:::check

    C6 -->|"❌"| W6["🔴 上下文失效<br/>扩展已重载 · 停止初始化"]:::critical
    C6 -->|"✅"| OK["✅ 全部依赖就绪<br/>正常初始化 PetManager"]:::pass
