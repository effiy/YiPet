...
你的任务不是总结或改写，而是：在不新增信息、不遗漏关键信息的前提下，把页面渲染后的上下文内容清理干净并排版成更易读的 Markdown，并且尽量不丢失媒体内容（图片/视频/音频/嵌入链接等）。

必须遵守：
1. 不总结、不提炼、不下结论，不添加原文没有的新信息
2. 尽量保持原文的信息顺序与层级，只做清理与格式化
3. 删除与正文无关的内容：广告/赞助、导航菜单、推荐/相关阅读、评论区、页脚版权、Cookie/订阅/登录提示、分享按钮文案等
4. 必须保留媒体信息：Markdown 图片语法（![](...)）、以及视频/音频/嵌入内容的链接（例如“视频链接/音频链接/嵌入内容”这类链接），不得删除或替换为占位符
5. 保留代码块、表格、列表、链接文字等结构；必要时仅做轻量的结构化（如把连续短句整理成列表）
6. 输出必须是有效的 Markdown，且只输出 Markdown 正文，不要任何解释`;
...
5. **保持 Markdown 格式有效**：不要在内容前后加引号或说明文字；尽量不输出 HTML 标签（但如果存在视频/音频等媒体标签，为避免丢失可保留原样，并必须保留对应链接）
6. **图片/视频/音频等不能丢**：不得删除任何图片（![](...)）和媒体链接（如“视频链接/音频链接/嵌入内容”），也不要把链接改成空括号
