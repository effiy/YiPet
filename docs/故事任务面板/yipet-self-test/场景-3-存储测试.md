...
```mermaid
flowchart LR
    DEV["修改 StorageHelper 清理策略"]:::pain --> TEST["npx vitest run<br/>tests/unit/storage.test.mjs"]:::action
    TEST --> CHECK{"配额超限清理<br/>是否正确?"}
    CHECK -->|"✅"| CONF["提交变更"]:::goal
    CHECK -->|"❌"| FIX["检查清理优先级<br/>和数据恢复"]:::fix
    FIX --> TEST
...
flowchart TB
    subgraph mock["chrome.storage.local mock (Map)"]
        GET["get(keys, callback)<br/>从 Map 读取 → callback(result)"]:::method
        SET["set(items, callback)<br/>写入 Map → callback()"]:::method
        REMOVE["remove(keys, callback)<br/>删除 Map entry → callback()"]:::method
        CLEAR["clear(callback)<br/>清空 Map → callback()"]:::method
        QUOTA["模拟配额超限<br/>Map.size > QUOTA_BYTES"]:::method
    end

    subgraph guards["存储访问守卫"]
        G1["isChromeStorageAvailable()<br/>检查 chrome.runtime.id"]:::guard
        G2["isQuotaError()<br/>检查 chrome.runtime.lastError"]:::guard
        G3["isContextInvalidatedError()<br/>检查扩展上下文"]:::guard
    end
