...
(function () {
  ...
  // ========== 常量定义 ==========
  const RENDER_MARKDOWN_ERROR_MESSAGE = '⚠️ 内容渲染失败，显示原始文本'
  ...
  proto._sanitizeUrl = function (url) {
    ...
    try {
      const prot = decodeURIComponent(s)
        .replace(/[^A-Za-z0-9/:]/g, '')
        .toLowerCase()
      if (prot.indexOf('javascript:') === 0 || prot.indexOf('vbscript:') === 0) {
        // 注意：data: URLs 在这里不被阻止，因为 _sanitizeImageSrc 会专门处理
        return ''
      }
    } catch (e) {
      return ''
  ...
  proto._sanitizeImageSrc = function (src) {
    ...
    // 允许 data URL 格式的图片（更宽松的正则）
    if (/^data:image\/[a-zA-Z0-9.+_-]+;base64,[a-z0-9+/\s=]+$/i.test(s)) {
      return s
    }
    ...
    const result = this._sanitizeUrl(s)

    // 调试日志
    if (result) {
      if (typeof console !== 'undefined' && console.debug) {
        console.debug('[PetManager] Image source sanitized', { original: src, sanitized: result })
      }
    } else {
      if (typeof console !== 'undefined' && console.warn) {
        console.warn('[PetManager] Image source blocked', { src })
      }
    }
  ...
  proto.renderMarkdown = function (markdown) {
    ...
    const logger = (typeof window !== 'undefined' && window.LoggerUtils && typeof window.LoggerUtils.getLogger === 'function')
      ? window.LoggerUtils.getLogger('message')
      : console

    try {
      logger.debug('开始渲染 Markdown', { length: markdown.length })
      ...
      if (typeof marked !== 'undefined') {
        ...
        const sanitized = this._sanitizeMarkdownHtml(rendered)
        logger.debug('Markdown 渲染成功', { renderedLength: sanitized.length })
        return sanitized
      } else {
        logger.warn('marked 不可用，返回转义的纯文本')
      ...
      logger.error('渲染 Markdown 失败:', errorInfo)

      // 显示友好的错误提示 + 原始文本
      const escapedMarkdown = this.escapeHtml(markdown)
      return `<div class="pet-markdown-error">
                <div class="pet-markdown-error__message">${this.escapeHtml(RENDER_MARKDOWN_ERROR_MESSAGE)}</div>
