...
> **导航**: [← 前驱](./场景-4-错误边界.md) | [回故事任务 →](./故事任务.md)
...
flowchart LR
    subgraph 配置层["配置层"]
        CFG["PET_CONFIG<br/>api.streamPromptUrl<br/>constants.RETRY.MAX_RETRIES<br/>constants.storageKeys"]:::static
    end
    subgraph 认证层["认证层"]
        TOK["TokenManager.getToken()<br/>三级回退→ string"]:::action
    end
    subgraph 请求层["请求层"]
        RC["RequestClient.request(opts)<br/>→ _fetchWithRetry<br/>→ _parseResponse"]:::action
        FETCH["fetch()"]:::external
    end
    subgraph 错误层["错误层"]
        EH["ErrorHandler<br/>categorize → shouldRetry → handle"]:::action
        FMT["formatError<br/>→ 结构化对象"]:::action
    end
    subgraph 存储层["存储层"]
        STO["StorageHelper.set/get<br/>→ chrome.storage.local"]:::action
        SM["SessionManager<br/>createSession → save → list → delete"]:::action
    end
...
```mermaid
flowchart TD
    S["开始"]:::start --> C["创建 SessionManager"]:::action
    C --> I["initialize()"]:::action
    I --> CS["createSession(id, pageInfo)<br/>→ session 含 key UUID · title · tags · createdAt"]:::action
    CS --> V1{"验证: session.key 为 UUID 格式"}
    V1 -->|"❌"| FAIL["失败"]:::fail
    V1 -->|"✅"| SAVE["saveSession(id)"]:::action
    SAVE --> LIST["getAllSessions()"]:::op
    LIST --> V2{"验证: 列表含刚创建的 session"}
    V2 -->|"❌"| FAIL
    V2 -->|"✅"| GET["getSession(id)"]:::op
    GET --> V3{"验证: 返回的 session 与创建时一致"}
    V3 -->|"❌"| FAIL
    V3 -->|"✅"| DUP["duplicateSession(id)"]:::action
    DUP --> V4{"验证: 副本 title 含 '(副本)' · messages 深拷贝 · tags 独立"}
    V4 -->|"❌"| FAIL
    V4 -->|"✅"| DEL["deleteSession(id)"]:::action
...
flowchart LR
    subgraph infra["基础设施"]
        ...
        VUE["vue.global.js<br/>CDN 运行时加载"]:::external
    end
    subgraph modules["加载链（8 模块）"]
        direction TB
        CFG["config.js"]:::source --> LOG["logger.js"]:::source
        LOG --> ERR["error.js"]:::source
        ERR --> TOK["token.js"]:::source
        TOK --> REQ["request.js"]:::source
        REQ --> APIM["ApiManager.js"]:::source
        APIM --> SES["SessionService.js"]:::source
        APIM --> FAQ["FaqService.js"]:::source
    end
    subgraph tests["集成测试"]
        INT["pipeline.test.mjs<br/>🆕 42 用例<br/>SessionService + FaqService"]:::test
        SW["sw.test.mjs<br/>🆕 8 用例<br/>MessageRouter 路由"]:::test
        CHAT["ChatWindow.test.mjs<br/>🆕 6 用例<br/>Vue 组件渲染"]:::test
    end
    SETUP --> modules --> tests
    VUE --> CHAT
```
