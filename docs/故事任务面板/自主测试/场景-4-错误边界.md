...
> **导航**: [← 前驱](./场景-3-存储测试.md) | [后继 →](./场景-5-集成测试.md)
...
flowchart TB
    ...
    subgraph CHECKS["分类检查链"]
        C0{"instanceof APIError ?"}:::decision
        C1{"isNetworkError ?"}:::decision
        C2{"isTimeoutError ?"}:::decision
        C3{"isAuthError ?"}:::decision
        C4{"isValidationError ?"}:::decision
        C5{"isRateLimitError ?"}:::decision
        C6{"默认"}:::decision
    end
    subgraph OUTPUT["分类输出"]
        O0["返回原 APIError"]:::result
        O1["new NetworkError(msg, ctx)"]:::result
        O2["new TimeoutError(msg, timeout, ctx)"]:::result
        O3["new AuthError(msg, ctx)"]:::result
        O4["new ValidationError(msg, fields, ctx)"]:::result
        O5["new RateLimitError(msg, retryAfter, ctx)"]:::result
        O6["new APIError(msg, 'UNKNOWN_ERROR', ctx)"]:::result
    end
    E --> C0
    C0 -->|"是"| O0
    C0 -->|"否"| C1
...
flowchart LR
    subgraph RULES["检测规则"]
        R1["error.name === 'TypeError'<br/>&& message.includes('Failed to fetch')"]:::rule
        R2["message.includes('NetworkError')"]:::rule
        R3["message.includes('net::ERR_')"]:::rule
        R4["message.includes('ECONNREFUSED')"]:::rule
    end
    NONE["error 为 falsy → false"]:::base
    NONE --> R1 -->|"任一满足"| TRUE["→ true"]:::true
    NONE --> R2 --> TRUE
    NONE --> R3 --> TRUE
    NONE --> R4 --> TRUE
```
...
```mermaid
flowchart LR
    R_A["error.name === 'AbortError'"]:::rule --> TRUE_A["→ true"]:::true
    R_B["message.includes('timeout')"]:::rule --> TRUE_A
    R_C["message.includes('Timeout')"]:::rule --> TRUE_A
    R_D["message.includes('ETIMEDOUT')"]:::rule --> TRUE_A
    NONE_T["error 为 falsy → false"]:::base
```
...
```mermaid
flowchart LR
    S401["error.status === 401"]:::rule --> TRUE_401["→ true"]:::true
    S403["error.status === 403"]:::rule --> TRUE_401
    UNAUTH["message.includes('Unauthorized')"]:::rule --> TRUE_401
    FORBID["message.includes('Forbidden')"]:::rule --> TRUE_401
    CN_AUTH["message.includes('认证')"]:::rule --> TRUE_401
    CN_FORB["message.includes('授权')"]:::rule --> TRUE_401
```
...
```mermaid
flowchart LR
    S400["error.status === 400"]:::rule --> TRUE_V["→ true"]:::true
    S422["error.status === 422"]:::rule --> TRUE_V
    VAL_MSG["message.includes('Validation')"]:::rule --> TRUE_V
    CN_VAL["message.includes('验证')"]:::rule --> TRUE_V
    CN_INV["message.includes('无效')"]:::rule --> TRUE_V
```
...
```mermaid
flowchart LR
    S429["error.status === 429"]:::rule --> TRUE_R["→ true"]:::true
    RATE_MSG["message.includes('Rate limit')"]:::rule --> TRUE_R
    TMR["message.includes('Too Many Requests')"]:::rule --> TRUE_R
    CN_RATE["message.includes('限流')"]:::rule --> TRUE_R
```
...
flowchart TD
    ...
    C0 -->|"是"| NO["→ false · 不重试"]:::false
    C0 -->|"否"| C1{"error instanceof NetworkError ?"}
    C1 -->|"是"| YES["→ true · 重试"]:::true
...
flowchart LR
    subgraph infra["基础设施"]
        SETUP["setup.mjs<br/>loadModule IIFE 加载<br/>beforeEach 清理 globalThis"]:::setup
    end
    subgraph src["被测试模块"]
        ERR["error.js<br/>6 种错误类<br/>ErrorHandler<br/>createError/formatError"]:::source
    end
    subgraph tests["测试"]
        ERR_T["error.test.mjs<br/>🆕 33 用例"]:::test
    end
    SETUP --> ERR
    ERR --> ERR_T
```
