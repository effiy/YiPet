        ...
        if (chrome.contextMenus) {
            ...
            chrome.contextMenus.onClicked.addListener(async (info, tab) => {
                if (info.menuItemId === 'yipet-toggle') {
                    try {
                        // 切换宠物显示
                        const { petManagerCore } = await import('./modules/pet/index.js');
                        const currentState = petManagerCore.getState();
                        await petManagerCore.setVisibility(!currentState.visible);
                    } catch (error) {
                ...
                try {
                    switch (command) {
                        ...
                        case 'take-screenshot':
                            const { screenshotManager } = await import('./modules/screenshot/index.js');
                            await screenshotManager.capture();
                            break;
                            
                        case 'open-chat':
                            const { chatManager } = await import('./modules/chat/index.js');
                            await chatManager.open();
                            break;
                    }
                } catch (error) {
