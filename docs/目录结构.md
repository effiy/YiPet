# 目录结构

> 温柔陪伴助手 - 完整的项目目录结构说明

---

## 📁 概述

本文档详细说明 YiPet 温柔陪伴助手的完整目录结构，帮助开发者快速了解项目组织。

---

## 🗂️ 完整目录结构

```
温柔陪伴助手/
│
├── manifest.json                          # 📋 Chrome 扩展清单文件，定义扩展的权限、脚本、图标等元信息
├── CLAUDE.md                              # 🤖 Claude Code AI 工具的项目指导文档
├── README.md                              # 📖 项目引导文档，指向完整文档
│
│
├── assets/                                # 🎨 样式、图片、图标资源
│   ├── icons/                             # 🔷 扩展图标资源
│   │   ├── icon.png                       # 🖼️ 扩展主图标
│   │   ├── icon128.png                    # 🖼️ 128x128 像素图标
│   │   ├── icon16.png                     # 🖼️ 16x16 像素图标
│   │   ├── icon32.png                     # 🖼️ 32x32 像素图标
│   │   └── icon48.png                     # 🖼️ 48x48 像素图标
│   │
│   ├── images/                            # 🐾 宠物角色图片资源
│   │   ├── 医生/                          # 👨‍⚕️ 医生角色
│   │   │   └── icon.png                   # 🖼️ 医生角色宠物图标
│   │   ├── 教师/                          # 👨‍🏫 教师角色
│   │   │   ├── icon.png                   # 🖼️ 教师角色宠物图标
│   │   │   └── run/                       # 🏃 跑步动画
│   │   │       ├── 1.png                  # 🖼️ 教师跑步动画第 1 帧
│   │   │       ├── 2.png                  # 🖼️ 教师跑步动画第 2 帧
│   │   │       └── 3.png                  # 🖼️ 教师跑步动画第 3 帧
│   │   ├── 甜品师/                        # 👨‍🍳 甜品师角色
│   │   │   └── icon.png                   # 🖼️ 甜品师角色宠物图标
│   │   └── 警察/                          # 👮 警察角色
│   │       └── icon.png                   # 🖼️ 警察角色宠物图标
│   │
│   └── styles/                            # 🎭 样式表文件
│       ├── base/                          # 🏗️ 基础样式
│       │   ├── animations.css             # ✨ 基础动画样式，包含宠物浮动、眨眼等动画
│       │   └── theme.css                  # 🎨 主题样式文件
│       ├── content.css                    # 🎨 内容脚本样式，定义宠物和聊天窗口的基础样式
│       ├── popup.css                      # 🎨 弹出页面样式
│       └── tailwind.css                   # 🎨 Tailwind CSS 工具类样式
│
├── core/                                  # 🔧 核心系统模块
│   ├── config.js                          # ⚙️ 集中配置文件，包含宠物、聊天窗口、API、环境等所有配置项
│   ├── bootstrap/                         # 🚀 引导/初始化代码
│   │   ├── bootstrap.js                   # 🚀 扩展启动引导程序
│   │   └── index.js                       # 🚪 引导模块入口
│   ├── constants/                         # 📌 常量定义
│   │   └── endpoints.js                   # 🔗 API 端点常量定义
│   ├── api/                               # 🔌 API 集成层
│   │   ├── core/                          # 🔧 API 核心管理器
│   │   │   └── ApiManager.js              # 🧠 API 请求管理器，处理所有 API 调用
│   │   ├── services/                      # 📋 API 服务
│   │   │   ├── FaqService.js              # ❓ FAQ CRUD 操作服务
│   │   │   └── SessionService.js          # 📦 会话管理服务
│   │   └── utils/                         # 🔧 API 工具类
│   │       ├── error.js                   # ⚠️ API 错误处理
│   │       ├── logger.js                  # 📝 API 日志记录
│   │       ├── request.js                 # 📨 请求辅助工具
│   │       └── token.js                   # 🔑 Token 管理工具
│   └── utils/                             # 🌐 全局工具模块
│       ├── api/                           # 🔌 API 特定辅助工具
│       ├── dom/                           # 🏗️ DOM 操作工具
│       │   └── domHelper.js               # 🛠️ DOM 操作辅助函数
│       ├── error/                         # ⚠️ 错误处理
│       │   └── errorHandler.js            # 🛡️ 全局错误处理器
│       ├── logging/                       # 📝 日志工具
│       │   └── loggerUtils.js             # 📋 日志记录工具
│       ├── media/                         # 🎬 媒体处理
│       │   └── imageResourceManager.js    # 🖼️ 图片资源管理器
│       ├── messaging/                     # 💬 消息传递
│       │   └── messageHelper.js           # 📨 Chrome Runtime 消息辅助工具
│       ├── runtime/                       # ⚙️ 运行时工具
│       │   ├── globalAccessor.js          # 🌍 全局变量访问器
│       │   └── moduleUtils.js             # 📦 模块加载工具
│       ├── session/                       # 📦 会话管理
│       │   └── sessionManager.js          # 🗄️ 会话存储管理器
│       ├── storage/                       # 💾 存储工具
│       │   └── storageUtils.js            # 📦 Chrome 存储包装器
│       ├── time/                          # ⏰ 时间工具
│       │   └── timeUtils.js               # 📅 时间/日期工具函数
│       └── ui/                            # 🎨 UI 工具
│           ├── loadingAnimation.js        # ⏳ 加载动画
│           ├── loadingAnimationMixin.js   # 🔄 加载动画混入
│           └── notificationUtils.js       # 🔔 通知系统工具
│
├── libs/                                  # 📚 第三方库
│   ├── marked.min.js                      # 📝 Markdown 渲染库，用于渲染聊天消息
│   ├── md5.js                             # 🔒 MD5 哈希计算库
│   ├── mermaid.min.js                     # 📊 Mermaid 图表渲染库
│   ├── turndown.js                        # 🔄 HTML 转 Markdown 转换库
│   └── vue.global.js                      # 💚 Vue.js 3 框架，用于构建 UI 组件
│
├── modules/                               # 🔧 功能模块（按功能组织）
│   ├── pet/                               # 🐾 宠物管理模块
│   │   ├── components/                    # 🖼️ Vue.js UI 组件
│   │   │   ├── chat/                      # 💬 聊天相关组件
│   │   │   │   ├── ChatHeader/            # 💬 聊天窗口头部组件
│   │   │   │   │   ├── index.html         # 📄 头部组件 HTML 模板
│   │   │   │   │   └── index.js           # 🧠 头部组件 Vue 应用逻辑
│   │   │   │   ├── ChatInput/             # ⌨️ 聊天输入框组件
│   │   │   │   │   ├── index.html         # 📄 输入框组件 HTML 模板
│   │   │   │   │   └── index.js           # 🧠 输入框组件 Vue 应用逻辑
│   │   │   │   ├── ChatMessages/          # 📨 聊天消息列表组件
│   │   │   │   │   ├── index.html         # 📄 消息列表组件 HTML 模板
│   │   │   │   │   └── index.js           # 🧠 消息列表组件 Vue 应用逻辑
│   │   │   │   └── ChatWindow/            # 🪟 主聊天窗口组件
│   │   │   │       ├── index.css          # 🎨 聊天窗口样式
│   │   │   │       ├── index.html         # 📄 聊天窗口主模板
│   │   │   │       ├── index.js           # 🧠 聊天窗口 Vue 应用主逻辑
│   │   │   │       └── hooks/             # 🎣 Vue 组合式函数钩子
│   │   │   │           ├── store.js        # 🗄️ 聊天窗口状态管理
│   │   │   │           ├── useComputed.js  # 🧮 计算属性钩子
│   │   │   │           └── useMethods.js   # ⚡ 方法钩子
│   │   │   │
│   │   │   ├── editor/                    # ✏️ 编辑器组件
│   │   │   │   └── SessionInfoEditor/     # ✏️ 会话信息编辑器组件
│   │   │   │       ├── index.html         # 📄 会话编辑器 HTML 模板
│   │   │   │       └── index.js           # 🧠 会话编辑器 Vue 应用逻辑
│   │   │   │
│   │   │   ├── manager/                   # 📋 管理器组件
│   │   │   │   ├── FaqManager/            # ❓ FAQ 管理器组件
│   │   │   │   │   ├── index.html         # 📄 FAQ 管理器 HTML 模板
│   │   │   │   │   └── index.js           # 🧠 FAQ 管理器 Vue 应用逻辑
│   │   │   │   ├── FaqTagManager/         # 🏷️ FAQ 标签管理器组件
│   │   │   │   │   ├── index.html         # 📄 FAQ 标签管理器 HTML 模板
│   │   │   │   │   └── index.js           # 🧠 FAQ 标签管理器 Vue 应用逻辑
│   │   │   │   └── SessionTagManager/     # 🏷️ 会话标签管理器组件
│   │   │   │       ├── index.html         # 📄 会话标签管理器 HTML 模板
│   │   │   │       └── index.js           # 🧠 会话标签管理器 Vue 应用逻辑
│   │   │   │
│   │   │   └── modal/                     # 🪟 弹窗组件
│   │   │       ├── AiSettingsModal/        # ⚙️ AI 设置弹窗组件
│   │   │       │   ├── index.html         # 📄 AI 设置弹窗 HTML 模板
│   │   │       │   └── index.js           # 🧠 AI 设置弹窗 Vue 应用逻辑
│   │   │       └── TokenSettingsModal/     # 🔑 API 令牌设置弹窗组件
│   │   │           ├── index.html         # 📄 令牌设置弹窗 HTML 模板
│   │   │           └── index.js           # 🧠 令牌设置弹窗 Vue 应用逻辑
│   │   │
│   │   ├── content/                       # 🐾 核心宠物管理逻辑
│   │   │   ├── core/                      # 🔧 核心实现
│   │   │   │   └── petManager.core.js     # 🧠 PetManager 核心实现，包含主类定义
│   │   │   ├── modules/                   # 📦 功能模块
│   │   │   │   ├── petManager.ai.js       # 🤖 AI 对话功能模块
│   │   │   │   ├── petManager.auth.js     # 🔐 认证和令牌管理模块
│   │   │   │   ├── petManager.editor.js   # ✏️ 会话编辑器模块
│   │   │   │   ├── petManager.io.js       # 📥📤 会话导入导出模块（已弃用）
│   │   │   │   ├── petManager.mermaid.js  # 📊 Mermaid 图表处理模块
│   │   │   │   ├── petManager.messaging.js # 💬 消息传递模块
│   │   │   │   ├── petManager.pageInfo.js # 📄 页面信息收集模块
│   │   │   │   ├── petManager.parser.js   # 🔍 消息解析模块
│   │   │   │   ├── petManager.robot.js    # 🤖 机器人动作模块
│   │   │   │   ├── petManager.roles.js    # 🎭 宠物角色配置模块
│   │   │   │   ├── petManager.session.js  # 📦 会话管理模块
│   │   │   │   ├── petManager.sessionEditor.js # ✏️ 会话编辑模块
│   │   │   │   └── petManager.tags.js     # 🏷️ 标签管理模块
│   │   │   ├── components/                # 🧩 内容脚本组件
│   │   │   │   ├── ai/                    # 🤖 AI 相关组件
│   │   │   │   ├── auth/                  # 🔐 认证相关组件
│   │   │   │   ├── chatWindow/            # 💬 聊天窗口组件
│   │   │   │   ├── sessionEditor/         # ✏️ 会话编辑器组件
│   │   │   │   └── sessionTagManager/     # 🏷️ 会话标签管理器组件
│   │   │   ├── petManager.chat.js         # 💬 聊天窗口管理
│   │   │   ├── petManager.chatUi.js       # 🎨 聊天 UI 交互
│   │   │   ├── petManager.drag.js         # ✋ 宠物拖拽功能
│   │   │   ├── petManager.events.js       # 🎉 事件处理
│   │   │   ├── petManager.media.js        # 🎬 媒体处理（图片等）
│   │   │   ├── petManager.message.js      # 💬 消息处理
│   │   │   ├── petManager.pet.js          # 🐾 宠物显示和动画
│   │   │   ├── petManager.screenshot.js   # 🖼️ 截图功能
│   │   │   ├── petManager.state.js        # 🗄️ 状态管理
│   │   │   ├── petManager.ui.js           # 🖼️ UI 组件管理
│   │   │   └── petManager.js              # 🚪 轻量入口/装配文件，组装各个功能模块
│   │   │
│   │   └── styles/                        # 🎨 宠物相关样式
│   │
│   ├── chat/                              # 💬 聊天功能模块
│   │   └── content/                        # 💬 聊天内容脚本
│   │
│   ├── faq/                               # ❓ FAQ 系统模块
│   │   └── content/                        # ❓ FAQ 内容脚本
│   │       ├── components/                 # 🧩 FAQ 组件
│   │       ├── faq.js                     # 🛠️ FAQ 管理主逻辑
│   │       └── tags.js                    # 🏷️ FAQ 标签管理
│   │
│   ├── session/                           # 📦 会话管理模块
│   │   └── page/                           # 📄 会话页面脚本
│   │
│   ├── screenshot/                        # 📸 截图功能模块
│   │   └── content/                        # 📸 截图内容脚本
│   │       └── petManager.screenshot.js   # 🖼️ 截图功能实现
│   │
│   ├── mermaid/                           # 📊 Mermaid 图表渲染模块
│   │   └── page/                           # 📊 Mermaid 页面脚本
│   │       ├── load-mermaid.js            # 📥 Mermaid 库加载器
│   │       ├── preview-mermaid.js         # 👁️ Mermaid 图表预览
│   │       └── render-mermaid.js         # 🎨 Mermaid 图表渲染
│   │
│   └── extension/                         # 🔌 Chrome 扩展系统
│       ├── background/                     # 🔙 后台服务 Worker
│       │   ├── index.js                   # 🚪 后台入口文件，处理扩展安装、更新和生命周期管理
│       │   ├── app/                        # 📱 应用注册
│       │   │   └── register.js            # 📋 Service Worker 注册
│       │   ├── bootstrap/                  # 🚀 引导模块
│       │   │   └── imports.js             # 📥 模块导入
│       │   ├── actions/                    # 📨 消息处理器
│       │   │   ├── extensionHandler.js    # 🛠️ 扩展操作消息处理器
│       │   │   ├── messageForwardHandler.js # 🔄 消息转发处理器
│       │   │   ├── petHandler.js         # 🐾 宠物相关消息处理器
│       │   │   ├── screenshotHandler.js  # 🖼️ 截图功能消息处理器
│       │   │   └── tabHandler.js         # 📑 标签页操作消息处理器
│       │   ├── messaging/                  # 💬 消息传递
│       │   │   └── messageRouter.js      # 🛠️ 消息路由器，分发消息到对应的处理器
│       │   ├── services/                   # 📋 后台服务
│       │   │   ├── injectionService.js    # 🛠️ 脚本注入服务
│       │   │   └── tabMessaging.js        # 💬 标签页消息传递服务
│       │   └── integrations/               # 🔗 外部集成
│       │       └── wework/                 # 🏢 企业微信集成
│       │           ├── weworkHandler.js    # 📨 企业微信消息处理器
│       │           └── weworkService.js    # 🛠️ 企业微信服务
│       │
│       ├── popup/                          # 🪟 弹出页面
│       │   ├── index.html                 # 📄 弹出页面 HTML
│       │   └── index.js                   # 🧠 弹出页面逻辑
│       │
│       ├── core/                           # 🔧 扩展核心模块
│       │   └── bootstrap/                  # 🚀 引导代码
│       │
│       └── content-scripts/                # 📄 内容脚本（由 manifest.json 配置）
│
└── docs/                                  # 📚 文档目录
    ├── README.md                          # 📖 完整的项目文档
    ├── API端点.md                        # 🔗 API 接口文档
    ├── 目录结构.md                        # 📁 本文件 - 项目目录结构说明
    ├── 架构设计.md                        # 🏗️ 项目架构和设计说明
    ├── 配置指南.md                        # ⚙️ 详细的配置说明（环境、API、存储）
    ├── 核心功能/                          # 🎯 核心功能详细说明
    │   ├── README.md                      # 📋 核心功能索引文件
    │   ├── 虚拟宠物展示.md                # 🐾 虚拟宠物展示功能
    │   ├── AI聊天界面.md                  # 💬 AI 聊天界面功能
    │   ├── 区域截图功能.md                # 📸 区域截图功能
    │   ├── 会话管理.md                    # 📦 会话管理功能
    │   ├── FAQ系统.md                     # ❓ FAQ 系统功能
    │   ├── Mermaid图表渲染.md             # 📊 Mermaid 图表渲染功能
    │   ├── 多种宠物角色.md                # 🎭 多种宠物角色功能
    │   └── 键盘快捷键.md                  # ⌨️ 键盘快捷键功能
    │
    ├── 组件库/                            # 🎨 Vue 组件库文档
    │   ├── README.md                      # 📋 组件库索引文件
    │   ├── 聊天组件/                      # 💬 聊天组件文档
    │   │   ├── ChatWindow.md              # 🪟 主聊天窗口组件
    │   │   ├── ChatHeader.md              # 💬 聊天窗口头部组件
    │   │   ├── ChatInput.md               # ⌨️ 聊天输入框组件
    │   │   ├── ChatMessage.md             # 💬 聊天消息组件
    │   │   └── ChatMessages.md            # 📨 聊天消息列表组件
    │   ├── 管理组件/                      # 📋 管理组件文档
    │   │   ├── FaqManager.md              # ❓ FAQ 管理器组件
    │   │   ├── FaqTagManager.md           # 🏷️ FAQ 标签管理器组件
    │   │   └── SessionTagManager.md       # 🏷️ 会话标签管理器组件
    │   ├── 编辑组件/                      # ✏️ 编辑组件文档
    │   │   └── SessionInfoEditor.md       # ✏️ 会话信息编辑器组件
    │   ├── 弹窗组件/                      # 🪟 弹窗组件文档
    │   │   ├── AiSettingsModal.md        # ⚙️ AI 设置弹窗组件
    │   │   └── TokenSettingsModal.md     # 🔑 API 令牌设置弹窗组件
    │   └── 通用组件/                      # 🔧 通用组件文档
    │       ├── LoadingSpinner.md          # ⏳ 加载动画组件
    │       └── Notification.md            # 🔔 通知组件
    │
    ├── 开发规范/                          # 🛠️ 完整的开发规范集合
    │   ├── README.md                      # 📋 开发规范索引文件
    │   ├── 代码审查.md                    # 👀 代码审查规范
    │   ├── 代码结构.md                    # 🏗️ 代码结构规范
    │   ├── 分支管理.md                    # 🌿 分支管理规范
    │   ├── 安全规范.md                    # 🔒 安全编码规范和最佳实践
    │   ├── 文档规范.md                    # 📚 文档编写和维护规范
    │   ├── 日志规范.md                    # 📝 日志记录和管理
    │   ├── 模块设计.md                    # 📦 模块设计规范
    │   ├── 测试策略.md                    # 🧪 测试策略规范
    │   ├── 测试规范.md                    # 🧪 单元测试、集成测试和验收测试
    │   ├── 版本控制.md                    # 🔖 版本控制规范
    │   ├── 编码规范.md                    # 💻 JavaScript 代码规范和风格指南
    │   ├── 部署规范.md                    # 🚀 扩展打包和部署流程
    │   ├── 错误处理规范.md                # ⚠️ 异常处理和错误报告
    │   ├── API文档.md                     # 📄 API 文档规范
    │   └── GIT提交规范.md                # 🎯 Git 提交信息规范和工作流程
    │
    └── devOps/                            # 👷 开发运维文档
        ├── needs/                         # 📋 需求文档
        │   ├── remove-export-chat-to-image.md  # 📋 移除导出聊天记录为图片功能的需求文档
        │   └── remove-session-zip-import-export.md  # 📋 移除会话 ZIP 导入导出功能的需求文档
        ├── plans/                         # 📝 实施计划文档
        │   ├── remove-export-chat-to-image.md  # 📝 移除导出聊天记录为图片功能的实施计划
        │   └── remove-session-zip-import-export.md  # 📝 移除会话 ZIP 导入导出功能的实施计划
        ├── specs/                         # 📋 功能设计文档
        │   ├── remove-export-chat-to-image.md  # 📋 移除导出聊天记录为图片功能的功能设计
        │   └── remove-session-zip-import-export.md  # 📋 移除会话 ZIP 导入导出功能的功能设计
        └── tests/                         # 🧪 测试报告文档
            ├── remove-export-chat-to-image.md  # 🧪 移除导出聊天记录为图片功能的验证与测试报告
            └── remove-session-zip-import-export.md  # 🧪 移除会话 ZIP 导入导出功能的验证与测试报告
```

---

## 📁 目录说明

### 顶级目录说明

| 目录/文件 | 说明 |
|-----------|------|
| `manifest.json` | Chrome 扩展清单文件，定义扩展的权限、脚本、图标等元信息 |
| `assets/` | 样式、图片、图标资源 |
| `core/` | 核心系统模块，包含 API 层、配置、常量和全局工具 |
| `libs/` | 第三方库（Vue、marked、mermaid 等） |
| `modules/` | 功能模块（按功能组织） |
| `docs/` | 文档目录 |

---

## 🎯 快速导航

### 核心代码位置
- **宠物管理**：`modules/pet/content/`
- **聊天组件**：`modules/pet/components/chat/`
- **后台脚本**：`modules/extension/background/`
- **API 层**：`core/api/`
- **全局工具**：`core/utils/`

### 文档位置
- **项目文档**：`README.md`
- **配置指南**：`docs/配置指南.md`
- **核心功能**：`docs/核心功能/`
- **组件库**：`docs/组件库/`
- **开发规范**：`docs/开发规范/`
- **DevOps**：`docs/devOps/`

---

## 📚 相关文档

- **[架构设计](./架构设计.md)** - 项目架构和关键模块说明
- **[配置指南](./配置指南.md)** - 详细的配置说明
- **[核心功能](./核心功能/)** - 核心功能文档
- **[组件库](./组件库/)** - Vue 组件库文档
- **[开发规范](./开发规范/)** - 完整的开发规范集合
- **[API端点](./API端点.md)** - API 接口文档

---

*最后更新：2026-03-19*
