...
| 默认角色配置 | `config.js: DEFAULTS.PET_ROLE: '教师'` | `DEFAULTS.PET_ROLE: 'teacher'` |
...
| `core/` | `core/config.js` | imports.js:22 | importScripts | 直接 | 反向依赖 | 同步修改 | 已闭合 | |
...
**关键约束**：由于是零构建项目，文件移动后所有引用路径必须手动同步更新。
