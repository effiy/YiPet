...
```mermaid
flowchart LR
    DEV["修改 RequestClient 重试策略"]:::pain --> TEST["npx vitest run<br/>tests/unit/api.test.mjs"]:::action
    TEST --> CHECK{"3次重试+退避<br/>是否正确?"}
    CHECK -->|"✅"| CONF["提交变更"]:::goal
    CHECK -->|"❌"| FIX["检查重试次数和间隔"]:::fix
    FIX --> TEST
...
flowchart TB
    subgraph client["RequestClient"]
        BUILD["_buildUrlWithParams(url, params)"]:::method
        OPTIONS["_buildFetchOptions(data, method)"]:::method
        FETCH["_fetchWithRetry(url, options, retries)"]:::method
        PARSE["_parseResponse(response)"]:::method
    end

    subgraph config["请求配置"]
        URL["url: string"]:::config
        METHOD["method: GET/POST/PUT/DELETE"]:::config
        DATA["data: object (for POST/PUT)"]:::config
        PARAMS["params: object (for GET)"]:::config
        TIMEOUT["timeout: number (default 30s)"]:::config
    end

    subgraph behavior["运行时行为"]
        RETRY["重试: 最多 3 次"]:::behavior
        BACKOFF["退避: 指数增长 (1s→2s→4s)"]:::behavior
        ABORT["超时: AbortController + setTimeout"]:::behavior
        DEDUP["去重: 5s 窗口内相同请求合并"]:::behavior
    end
