...
> **导航**: [← 故事任务](./故事任务.md) · [场景-2-接口测试 →](./场景-2-接口测试.md)
...
flowchart TB
    ...
    subgraph 被测试模块["被测试模块"]
        direction TB
        CFG["config.js<br/>DEFAULT_CONFIG + ENDPOINTS<br/>buildUrl / buildQueryParams<br/>环境检测 + PET_CONFIG 导出"]:::core
        TOK["token.js<br/>TokenManager 类<br/>三级回退：环境变量→storage→空<br/>validateToken 格式校验"]:::exec
        STO["storageUtils.js<br/>StorageUtils 类<br/>loadFromChromeStorage / saveToChromeStorage<br/>normalizeState / 配额处理"]:::exec
    end
    subgraph 测试层["测试层"]
        direction TB
        CFG_T["config.test.mjs<br/>DEFAULT_CONFIG 结构验证<br/>buildUrl 参数替换<br/>buildQueryParams URLSearchParams<br/>环境检测开关"]:::exec
        TOK_T["token.test.mjs<br/>构造参数 · getToken 三级<br/>saveToken · clearToken<br/>validateToken 边界<br/>TokenUtils 便利方法"]:::exec
        STO_T["storage.test.mjs<br/>chrome.storage mock get/set<br/>配额模拟 · 清理<br/>normalizeState 默认值"]:::exec
    end
    CFG --> CFG_T
    TOK --> TOK_T
    STO --> STO_T
...
flowchart TB
    ...
    subgraph setup["vitest Setup 层"]
        direction TB
        SET["setup.mjs<br/>chrome API mock 注入<br/>fetch mock 初始化"]:::exec
        MOD["loadModule helper<br/>Function 构造器加载 IIFE<br/>注入 globalThis/window/chrome"]:::exec
    end
    subgraph unit["Unit 测试层"]
        direction LR
        subgraph cfg["配置测试"]
            direction TB
            C1["DEFAULT_CONFIG<br/>结构完整性"]:::exec
            C2["buildUrl<br/>参数替换"]:::exec
            C3["buildQueryParams<br/>查询串构造"]:::exec
            C4["环境检测<br/>window/process/默认"]:::exec
        end
        subgraph tok["Token 测试"]
            direction TB
            T1["构造参数<br/>storageKey 自定义"]:::exec
            T2["getToken<br/>三级回退链"]:::exec
            T3["validateToken<br/>格式与边界"]:::exec
            T4["TokenUtils<br/>便利方法"]:::exec
        end
        subgraph sto["存储测试"]
            direction TB
            S1["get/set/remove<br/>chrome.storage.local"]:::exec
            S2["配额超限<br/>清理重试"]:::exec
            S3["上下文失效<br/>优雅降级"]:::exec
        end
    end
    setup --> unit
```
...
flowchart LR
    ...
    subgraph infra["基础设施"]
        ...
        VITEST["vitest.config.js<br/>environment: jsdom<br/>globals: true"]:::core
    end
    subgraph unit_tests["单元测试"]
        direction TB
        CFG_T["config.test.mjs<br/>30 用例"]:::exec
        TOK_T["token.test.mjs<br/>30 用例"]:::exec
        STO_T["storage.test.mjs<br/>70 用例"]:::exec
    end
    subgraph core_tests["IIFE 模块测试"]
        direction TB
        TOK2["core/utils/token.test.mjs<br/>8 用例"]:::exec
        STO2["core/utils/storage.test.mjs<br/>5 用例"]:::exec
        MCK["chrome.test.mjs<br/>4 用例"]:::exec
    end
    VITEST --> SETUP
    SETUP --> unit_tests
    SETUP --> core_tests
...
| 维度 | 评估 | 说明 |
|------|:---:|------|
| E1 功能正确性 | 10/10 | 配置/Token/存储核心逻辑 100% 验证通过 |
...
| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| v1.0.0 | 2026-06-02 | coder | 初始版本：核心逻辑测试场景文档 |
