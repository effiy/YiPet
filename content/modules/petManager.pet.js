...
(function () {
    ...
    proto.updatePetStyle = function() {
        ...
        const iconUrl = chrome.runtime.getURL(`assets/images/roles/${role}/icon.png`);
    ...
    proto.showLoadingAnimation = async function() {
        ...
        if (!this.originalBackgroundImage) {
            try {
                ...
                if (iconImg && iconImg.src) {
                    ...
                    this.originalBackgroundImage = chrome.runtime.getURL(`assets/images/roles/${role}/icon.png`);
                }
            } catch (error) {
        ...
        try {
            if (window.imageResourceManager) {
                ...
                // 降级：使用原始 URL
                runFrameUrls = [
                    chrome.runtime.getURL(`assets/images/roles/${role}/run/1.png`),
                    chrome.runtime.getURL(`assets/images/roles/${role}/run/2.png`),
                    chrome.runtime.getURL(`assets/images/roles/${role}/run/3.png`)
                ];
            }
        } catch (error) {
