...
```mermaid
flowchart TB
    START["新人加入"]:::start --> ENV["① 环境搭建<br/>~10 分钟"]:::step
    
    ENV --> OVERVIEW["② 架构概览<br/>阅读 CLAUDE.md + 场景 1-4"]:::step
    OVERVIEW --> DEBUG["③ 调试方法<br/>掌握 3 种 DevTools 调试入口"]:::step
    
    DEBUG --> PATTERNS["④ 代码模式<br/>IIFE · Vue Options API · Manifest 顺序"]:::step
    PATTERNS --> FIRST["⑤ 完成首修<br/>选择一个简单任务"]:::step
    
    FIRST --> ADVANCED["⑥ 进阶<br/>Service Worker · 消息路由 · Mermaid"]:::step
...
flowchart LR
    subgraph setup["环境搭建 3 步"]
        S1["① 克隆项目<br/>git clone + cd"]:::action
        S2["② 加载扩展<br/>Chrome → 扩展程序 →<br/>开发者模式 →<br/>加载已解压的扩展<br/>→ 选择项目根目录"]:::action
        S3["③ 验证<br/>打开任意网页<br/>看到宠物图标"]:::action
        S1 --> S2 --> S3
    end
...
flowchart TB
    subgraph debug1["入口 1: Content Script Debugging"]
        D1A["打开任意网页<br/>右键 → 检查"]:::action
        D1B["Console 标签<br/>输入 petManager"]:::result
        D1C["Sources 标签<br/>找到 content scripts → 项目文件<br/>可设置断点"]:::action
    end

    subgraph debug2["入口 2: Service Worker Debugging"]
        D2A["chrome://extensions<br/>找到 YiPet 卡片"]:::action
        D2B["点击 Service Worker 链接<br/>检查视图: 后台页"]:::result
        D2C["Console 标签<br/>查看 SW 日志 · 消息路由"]:::result
    end

    subgraph debug3["入口 3: Popup Debugging"]
        D3A["点击工具栏 YiPet 图标<br/>弹出 Popup"]:::action
        D3B["右键 Popup 窗口 → 检查"]:::action
        D3C["独立 DevTools 窗口<br/>调试 PopupController"]:::result
    end
...
flowchart LR
    subgraph iife_pattern["IIFE 模块注册模式"]
        P1["(function(root) {<br/>  class MyModule { ... }<br/>  root.MyModule = MyModule<br/>})(globalThis)"]:::code
    end

    subgraph vue_pattern["Vue Options API 模式"]
        P2["const app = Vue.createApp({<br/>  template: '...',<br/>  data() { return {...} },<br/>  methods: { ... },<br/>  mounted() { ... }<br/>})<br/>app.mount('#container')"]:::code
    end

    subgraph prototype_pattern["PetManager 子模块挂载"]
        P3["const proto = window.PetManager.prototype<br/>proto.myMethod = function() {<br/>  // 通过 this 访问 PetManager 实例属性<br/>}"]:::code
    end

    subgraph config_pattern["配置端点注册"]
        P4["// core/config.js<br/>ENDPOINTS.MY_ENDPOINTS = {<br/>  LIST: '/my-resource',<br/>  CREATE: '/my-resource'<br/>}"]:::code
    end
...
```mermaid
flowchart TB
    T1_S["开始"]:::start --> T1_1["① 创建组件目录<br/>modules/pet/components/MyComponent/"]:::action
    T1_1 --> T1_2["② 创建 index.js<br/>Vue.createApp({ template, data, methods })"]:::action
    T1_2 --> T1_3["③ 创建 index.html 模板文件（如需要）"]:::action
    T1_3 --> T1_4["④ 在 manifest.json<br/>content_scripts.js 数组末尾添加路径"]:::action
    T1_4 --> T1_5["⑤ 在 manifest.json<br/>web_accessible_resources 中添加 index.html（如使用）"]:::action
    T1_5 --> T1_6["⑥ 在 InjectionService.CONTENT_SCRIPT_FILES<br/>末尾添加路径（与 manifest 同步）"]:::action
    T1_6 --> T1_7["⑦ 在 PetManager 中调用<br/>proto.initMyComponent = function() { ... }"]:::action
    T1_7 --> T1_E["完成"]:::done

    classDef start fill:#c8e6c9,stroke:#2e7d32
    classDef action fill:#e8f5e9,stroke:#2e7d32
    classDef done fill:#e3f2fd,stroke:#1565c0
```
...
flowchart TB
    subgraph start_here["从哪里开始"]
        SH1["manifest.json<br/>理解扩展声明 · 权限 · 加载顺序"]:::highlight
        SH2["core/config.js<br/>理解配置体系 · 常量 · 端点"]:::highlight
        SH3["core/bootstrap/bootstrap.js<br/>理解 StorageHelper · 位置工具"]:::highlight
        SH4["core/bootstrap/index.js<br/>理解 Content Script 启动流程"]:::highlight
        SH5["modules/pet/content/core/petManager.core.js<br/>理解 PetManager 类 · 状态初始化"]:::highlight
    end

    subgraph next["下一步深入"]
        N1["core/utils/api/token.js<br/>TokenManager 三级降级"]:::normal
        N2["core/utils/api/request.js<br/>RequestClient · fetch + 重试"]:::normal
        N3["core/api/core/ApiManager.js<br/>拦截器链 · X-Token 注入"]:::normal
        N4["modules/pet/content/petManager.chat.js<br/>聊天核心逻辑入口"]:::normal
        N5["modules/extension/background/app/register.js<br/>Service Worker 消息路由注册"]:::normal
    end

    subgraph deep["高级主题"]
        D1["modules/pet/content/ai/<br/>流式 API · Prompt 构建"]:::advanced
        D2["modules/pet/components/chat/<br/>Vue 聊天组件体系"]:::advanced
        D3["modules/extension/background/messaging/<br/>MessageRouter 实现"]:::advanced
        D4["cdn/markdown/<br/>Mermaid 懒加载编排"]:::advanced
        D5["modules/extension/background/integrations/wework/<br/>企微集成"]:::advanced
    end

    start_here --> next --> deep

    classDef highlight fill:#c8e6c9,stroke:#2e7d32
