...
```mermaid
flowchart LR
    DEV["计划修改 ApiManager<br/>添加新的拦截器"]:::pain --> CHECK{"查阅消费矩阵<br/>谁依赖 ApiManager?"}
    CHECK --> FIND["发现: SessionService<br/>FaqService 都依赖 ApiManager"]:::risk
    FIND --> ASSESS["评估: 新增拦截器<br/>不影响现有接口签名<br/>破坏性: 无"]:::action
...
flowchart TB
    subgraph change["变更点: core/config.js ENDPOINTS"]
        C1["修改 /api/v1/sessions → /api/v2/sessions"]:::change
    end

    subgraph wave1["第一波: 直接消费者"]
        W1["ApiManager._buildUrl()"]:::impact1
        W2["SessionService — 所有 CRUD 方法"]:::impact1
        W3["FaqService — 所有 CRUD 方法"]:::impact1
    end

    subgraph wave2["第二波: 间接消费者"]
        W4["PetManager.session.crud.js — 会话操作"]:::impact2
        W5["PetManager — FAQ 管理"]:::impact2
        W6["FaqManager (Vue) — FAQ UI"]:::impact2
    end

    subgraph wave3["第三波: UI 层"]
        W7["ChatWindow — 会话列表显示"]:::impact3
        W8["SessionTagManager — 标签筛选"]:::impact3
    end
    ...
    classDef change fill:#ffcdd2,stroke:#c62828
    classDef impact1 fill:#fff9c4,stroke:#f9a825
    classDef impact2 fill:#e3f2fd,stroke:#1565c0
    classDef impact3 fill:#e0f7fa,stroke:#00838f
```
